...
public class DebugDraw : Node
{
	...
	/// </summary>
	public static FPSGraphTextFlags FPSGraphShowTextFlags { get; set; } = FPSGraphTextFlags.Avarage | FPSGraphTextFlags.Current | FPSGraphTextFlags.Max | FPSGraphTextFlags.Min;
...
#pragma warning restore CA1822 // Mark members as static
	...
	/// <param name="color">Color</param>
	/// <param name="duration">Duration of existence in seconds</param>
	public static void DrawCameraFrustum(Godot.Collections.Array cameraFrustum, Color? color = null, float duration = 0f)
	{
#if DEBUG
...
namespace DebugDrawInteranlFunctionality
{
	...
	class FPSGraph
	{
		...
		public void Draw(CanvasItem ci, Font font, Vector2 viewportSize)
		{
			...
			if ((DebugDraw.FPSGraphShowTextFlags & DebugDraw.FPSGraphTextFlags.Max) == DebugDraw.FPSGraphTextFlags.Max)
				ci.DrawString(font, pos + new Vector2(4, max_height - 1), max_text, DebugDraw.FPSGraphTextColor);
			if ((DebugDraw.FPSGraphShowTextFlags & DebugDraw.FPSGraphTextFlags.Avarage) == DebugDraw.FPSGraphTextFlags.Avarage)
				ci.DrawString(font, pos + new Vector2(4, graphSize.y * 0.5f + avg_height * 0.5f - 2), avg_text, DebugDraw.FPSGraphTextColor);
			if ((DebugDraw.FPSGraphShowTextFlags & DebugDraw.FPSGraphTextFlags.Min) == DebugDraw.FPSGraphTextFlags.Min)
				ci.DrawString(font, pos + new Vector2(4, graphSize.y - 3), min_text, DebugDraw.FPSGraphTextColor);
			if ((DebugDraw.FPSGraphShowTextFlags & DebugDraw.FPSGraphTextFlags.Current) == DebugDraw.FPSGraphTextFlags.Current)
				ci.DrawString(font, pos + new Vector2(graphSize.x - cur_size.x, graphSize.y * 0.5f + cur_size.y * 0.5f - 2), cur_text, DebugDraw.FPSGraphTextColor);
		}
	}
	...
	class MultiMeshContainer
	{
		...
		readonly Dictionary<MultiMeshInstance, HashSet<DelayedRendererInstance>> all_mmi_with_values = new Dictionary<MultiMeshInstance, HashSet<DelayedRendererInstance>>();
		...
		public MultiMeshContainer(Node root, Action<int> onObjectRendered)
		{
			...
			// Create Meshes
			_mmi_cubes.Multimesh.Mesh = CreateMesh(Mesh.PrimitiveType.Lines, DebugDrawImplementation.CubeVertices, DebugDrawImplementation.CubeIndices);
			_mmi_cubes_centered.Multimesh.Mesh = CreateMesh(Mesh.PrimitiveType.Lines, DebugDrawImplementation.CenteredCubeVertices, DebugDrawImplementation.CubeIndices);
			_mmi_arrowheads.Multimesh.Mesh = CreateMesh(Mesh.PrimitiveType.Lines, DebugDrawImplementation.ArrowheadVertices, DebugDrawImplementation.ArrowheadIndices);
			_mmi_billboard_squares.Multimesh.Mesh = CreateMesh(Mesh.PrimitiveType.Triangles, DebugDrawImplementation.CenteredSquareVertices, DebugDrawImplementation.SquareIndices);
			_mmi_positions.Multimesh.Mesh = CreateMesh(Mesh.PrimitiveType.Lines, DebugDrawImplementation.PositionVertices, DebugDrawImplementation.PositionIndices);
			_mmi_spheres.Multimesh.Mesh = CreateMesh(Mesh.PrimitiveType.Lines, DebugDrawImplementation.CreateSphereLines(6, 6, 0.5f, Vector3.Zero));
			_mmi_cylinders.Multimesh.Mesh = CreateMesh(Mesh.PrimitiveType.Lines, DebugDrawImplementation.CreateCylinderLines(52, 0.5f, 1, Vector3.Zero, 4));
		}

		MultiMeshInstance CreateMMI(Node root, string name)
		{
			var mmi = new MultiMeshInstance() { MaterialOverride = new SpatialMaterial() { FlagsUnshaded = true, VertexColorUseAsAlbedo = true } };
			mmi.Multimesh = new MultiMesh() { ColorFormat = MultiMesh.ColorFormatEnum.Float, CustomDataFormat = MultiMesh.CustomDataFormatEnum.None, TransformFormat = MultiMesh.TransformFormatEnum.Transform3d, };
			mmi.Name = name;
			root.AddChild(mmi);
	...
	class DebugDrawImplementation : IDisposable
	{
			...
			set
			{
				...
				if (value == null)
				{
					if (!connected_internal)
						CanvasItemInternal.Connect("draw", debugDraw, nameof(DebugDraw.OnCanvaItemDraw), new Godot.Collections.Array { CanvasItemInternal });
					if (connected_custom)
...

				{
					...
					if (!connected_custom)
						value.Connect("draw", debugDraw, nameof(DebugDraw.OnCanvaItemDraw), new Godot.Collections.Array { value });
				}
				_customCanvas = value;
			}
		}
		...
		public void Ready()
		{
			...
			_immediateGeometry = new ImmediateGeometry() { Name = nameof(_immediateGeometry), MaterialOverride = new SpatialMaterial() { FlagsUnshaded = true, VertexColorUseAsAlbedo = true } };
			...
			{ // Text drawing
				...
				switch (DebugDraw.TextBlockPosition)
				{
					case DebugDraw.BlockPosition.LeftTop:
						...
						break;
					case DebugDraw.BlockPosition.RightTop:
						pos = new Vector2(vp_size.x - DebugDraw.TextBlockOffset.x, DebugDraw.TextBlockOffset.y);
						size_mul = -1;
						break;
					case DebugDraw.BlockPosition.LeftBottom:
						pos = new Vector2(DebugDraw.TextBlockOffset.x, vp_size.y - DebugDraw.TextBlockOffset.y - line_height * count);
						size_mul = 0;
						break;
					case DebugDraw.BlockPosition.RightBottom:
						pos = new Vector2(vp_size.x - DebugDraw.TextBlockOffset.x, vp_size.y - DebugDraw.TextBlockOffset.y - line_height * count);
						size_mul = -1;
						break;
				}
				...
				foreach (var g in _textGroups.OrderBy(g => g.GroupPriority))
				{
					...
					foreach (var t in g.ShowTitle ? a.Prepend(new KeyValuePair<string, DelayedText>(g.Title ?? "", null)) : a)
					{
						...
						ci.DrawRect(new Rect2(new Vector2(pos.x + size_right_revert, pos.y), new Vector2(size.x + DebugDraw.TextPadding.x * 2, line_height)), DebugDraw.TextBackgroundColor);
						...
						else
						{
							...
							ci.DrawString(_font, new Vector2(pos.x + font_offset.x + size_right_revert, pos.y + font_offset.y), text.Substring(0, _keyLength), g.GroupColor);
							ci.DrawString(_font, new Vector2(pos.x + font_offset.x + size_right_revert + _font.GetStringSize(textSep).x, pos.y + font_offset.y), text.Substring(_keyLength), t.Value.ValueColor);
						}
						pos.y += line_height;
					}
				}
		...
		public void DrawCameraFrustumInternal(ref Godot.Collections.Array cameraFrustum, ref Color? color, float duration)
		{
			if (!DebugDraw.DebugEnabled) return;
